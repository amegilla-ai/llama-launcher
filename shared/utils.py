...
def parse_help_output(help_text):
    """Parse llama.cpp help output to extract parameters and descriptions."""
    params = {}
...
def generate_param_references():
    ...
    try:
        ...
        server_help = subprocess.run([server_bin, "--help"], 
                                   capture_output=True, text=True, timeout=10)
        cli_help = subprocess.run([cli_bin, "--help"], 
                                capture_output=True, text=True, timeout=10)
        
        if server_help.returncode != 0 or cli_help.returncode != 0:
            return False, "Failed to get help output from binaries"
            
        # Parse parameters
        server_params = parse_help_output(server_help.stdout)
        cli_params = parse_help_output(cli_help.stdout)
        
        # Categorize parameters
        all_server = set(server_params.keys())
        all_cli = set(cli_params.keys())
        
        common_params = all_server & all_cli
        server_only = all_server - all_cli
        cli_only = all_cli - all_server
        
        # Build reference data
        references = {
            "common": {param: server_params[param] for param in common_params},
            "server": {param: server_params[param] for param in server_only},
            "cli": {param: cli_params[param] for param in cli_only}
        }
        ...
        return True, f"Generated {len(common_params)} common, {len(server_only)} server, {len(cli_only)} CLI parameters"
